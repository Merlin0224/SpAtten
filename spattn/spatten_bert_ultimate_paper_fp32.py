...
def split_fp32_high16_low16(x):
    if x.dtype != torch.float32:
        x = x.float()

    x = x.contiguous()
    bits = x.view(torch.int32)
    high16_mask = torch.tensor(-65536, device=x.device, dtype=torch.int32)
    high_bits = torch.bitwise_and(bits, high16_mask)
    x_high = high_bits.view(torch.float32)
    x_low = x - x_high
    return x_high, x_low


def prepare_progressive_k_paper_semantics(key_layer):
    return split_fp32_high16_low16(key_layer)
...
class SpattenBertSelfAttention(BertSelfAttention):
    ...
    def forward(
        ...
        if self.enable_prog_quant and self.enable_v_prune:
            ...
            k_msb, k_lsb = prepare_progressive_k_paper_semantics(key_layer)
...
def main():
    ...
    print(f" SpAtten Paper-Semantics FP32 Integration Running on: {device} \n")
