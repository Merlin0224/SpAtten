...
# Experimental branch file:
# Keeps non-mainline dynamic V-threshold strategy isolated from mainline implementation.
...
class SpattenQwen3Attention(Qwen3Attention):
    def __init__(self, config, layer_idx):
        ...
        self.enable_dynamic_v_threshold = False
        self.dynamic_v_layer_scale = 0.0
        self.dynamic_v_seq_scale = 0.0
        self.dynamic_v_max = 0.20
    ...
    def _effective_v_threshold(self, context_len: int):
        base = float(self.v_threshold)
        if not self.enable_dynamic_v_threshold:
            return base

        total_layers = max(1, int(getattr(self.config, "num_hidden_layers", 1)))
        layer_ratio = float(self.layer_idx) / float(max(1, total_layers - 1))

        seq_term = 0.0
        if context_len > 2048:
            seq_term = float(context_len - 2048) / 2048.0

        value = base + self.dynamic_v_layer_scale * layer_ratio + self.dynamic_v_seq_scale * seq_term
        value = max(base, value)
        value = min(float(self.dynamic_v_max), value)
        return float(value)

    def _decode_v_prune_fast_path(self, query_states, key_states, value_states, attention_mask, v_threshold):
        attn_scores = torch.matmul(query_states, key_states.transpose(2, 3)) * self.scaling
        aligned_mask = self._align_attention_mask_for_scores(attention_mask, attn_scores)
        if aligned_mask is not None:
            attn_scores = attn_scores + aligned_mask
        attn_probs = torch.softmax(attn_scores, dim=-1, dtype=torch.float32).to(query_states.dtype)
        pruned_probs = torch.where(attn_probs > v_threshold, attn_probs, torch.zeros_like(attn_probs))
    ...
    def forward(
        ...
        **kwargs,
        ...
        effective_v_threshold = self._effective_v_threshold(key_states.size(-2))
        ...
        if is_decode_step and self.enable_decode_fast_path:
            ...
            context_layer = triton_fused_spatten_ultimate_causal(
                ...
                v_threshold=effective_v_threshold,
...
def configure_spatten_qwen3_model(
    ...
    enable_dynamic_v_threshold=False,
    dynamic_v_layer_scale=0.0,
    dynamic_v_seq_scale=0.0,
    dynamic_v_max=0.20,
    meta_overrides=None,
):
    ...
    for layer_idx, layer in enumerate(model.layers):
        ...
        new_attn.enable_dynamic_v_threshold = enable_dynamic_v_threshold
        new_attn.dynamic_v_layer_scale = dynamic_v_layer_scale
        new_attn.dynamic_v_seq_scale = dynamic_v_seq_scale
        new_attn.dynamic_v_max = dynamic_v_max
        if meta_overrides:
