...
# Experimental file for non-mainline exploration:
# use_cache path optimization with explicit prefill/decode separation
# and decode-side KV participation range controls.
...
class SpattenQwen3Attention(Qwen3Attention):
    def __init__(self, config, layer_idx):
        ...
        self.prefill_sdpa_max_seq_len = 0
        self.enable_gqa_active_qkv_prefill = True
        self.enable_prefill_static_reuse = True
        self.enable_decode_kv_window = False
        self.decode_kv_window_size = 0
        self.decode_kv_prefix_keep = 0
    ...
    def _maybe_slice_kv_for_decode(self, key_states, value_states, attention_mask):
        if not self.enable_decode_kv_window:
            return key_states, value_states, attention_mask

        kv_len = key_states.size(-2)
        window = int(self.decode_kv_window_size)
        if window <= 0 or kv_len <= window:
            return key_states, value_states, attention_mask

        prefix_keep = max(0, int(self.decode_kv_prefix_keep))
        prefix_keep = min(prefix_keep, max(0, kv_len - window))
        tail_keep = min(window, kv_len - prefix_keep)

        if prefix_keep > 0:
            prefix_idx = torch.arange(prefix_keep, device=key_states.device, dtype=torch.long)
            tail_idx = torch.arange(kv_len - tail_keep, kv_len, device=key_states.device, dtype=torch.long)
            keep_idx = torch.cat([prefix_idx, tail_idx], dim=0)
        else:
            keep_idx = torch.arange(kv_len - tail_keep, kv_len, device=key_states.device, dtype=torch.long)

        key_states = key_states.index_select(2, keep_idx)
        value_states = value_states.index_select(2, keep_idx)
        if attention_mask is not None:
            attention_mask = attention_mask.index_select(-1, keep_idx)
        return key_states, value_states, attention_mask
    ...
    def forward(
        ...
        **kwargs,
        ...
        if is_decode_step:
            key_states, value_states, attention_mask = self._maybe_slice_kv_for_decode(
                key_states,
                value_states,
                attention_mask,
            )

        use_prefill_sdpa_path = self.enable_prefill_sdpa_fast_path
        if (
            not is_decode_step
            and not use_prefill_sdpa_path
            and self.prefill_sdpa_max_seq_len > 0
            and query_states.size(-2) <= self.prefill_sdpa_max_seq_len
        ):
            use_prefill_sdpa_path = True
...
def spatten_qwen3_model_forward(
    ...
    **kwargs,
    ...
    prefill_static_reuse_enabled = all(
        getattr(layer.self_attn, "enable_prefill_static_reuse", True)
    ...
    if not token_prune_enabled and prefill_static_reuse_enabled:
