...
from spattn.spatten_qwen3_bf16_msb_exp_dynamic_v import (
...
def run_spatten_variant(
    ...
    spatten_sdpa_prefill_max_seq_len,
    enable_dynamic_v_threshold,
    dynamic_v_layer_scale,
    dynamic_v_seq_scale,
    dynamic_v_max,
):
    inputs, input_source = build_inputs_local_or_synthetic(
    ...
    model = configure_spatten_qwen3_model(
        ...
        prefill_sdpa_max_seq_len=spatten_sdpa_prefill_max_seq_len,
        enable_dynamic_v_threshold=enable_dynamic_v_threshold,
        dynamic_v_layer_scale=dynamic_v_layer_scale,
        dynamic_v_seq_scale=dynamic_v_seq_scale,
        dynamic_v_max=dynamic_v_max,
...
def main():
    parser = argparse.ArgumentParser(
        description="Qwen3 attention backend comparison (experimental dynamic-v-threshold branch)."
    )
    ...
    parser.add_argument("--spatten-sdpa-prefill-max-seq-len", type=int, default=0)
    parser.add_argument("--enable-dynamic-v-threshold", action="store_true")
    parser.add_argument("--dynamic-v-layer-scale", type=float, default=0.0)
    parser.add_argument("--dynamic-v-seq-scale", type=float, default=0.0)
    parser.add_argument("--dynamic-v-max", type=float, default=0.20)
    ...
    print(f"SpAtten SDPA prefill max seq len: {args.spatten_sdpa_prefill_max_seq_len}")
    print(
        f"Dynamic V threshold: {args.enable_dynamic_v_threshold} "
        f"(layer_scale={args.dynamic_v_layer_scale}, seq_scale={args.dynamic_v_seq_scale}, max={args.dynamic_v_max})"
    )
    print(
    ...
    for seq_len in seq_lens:
        ...
        for label, mode in spatten_modes:
            result = run_spatten_variant(
                ...
                spatten_sdpa_prefill_max_seq_len=args.spatten_sdpa_prefill_max_seq_len,
                enable_dynamic_v_threshold=args.enable_dynamic_v_threshold,
                dynamic_v_layer_scale=args.dynamic_v_layer_scale,
                dynamic_v_seq_scale=args.dynamic_v_seq_scale,
                dynamic_v_max=args.dynamic_v_max,
            )
    ...
    output_path = output_dir / f"model_seq_attention_backend_compare_qwen3_exp_dynamic_v_{timestamp}.json"
    payload = {
        "timestamp": timestamp,
